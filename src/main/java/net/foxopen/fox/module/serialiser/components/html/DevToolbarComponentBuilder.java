...
public class DevToolbarComponentBuilder extends ComponentBuilder<HTMLSerialiser, EvaluatedPresentationNode> {
  ...
  public void buildComponent(SerialisationContext pSerialisationContext, HTMLSerialiser pSerialiser, EvaluatedPresentationNode pEvalNode) {
    ...
    try {
      ...
      // Contexts Link
      pSerialiser.append("<li id=\"dev-toolbar-contexts\"><a href=\"#\" class=\"icon-link\">XPath Info</a></li> \n");
...
//    lDOMViewURIBuilder.setParam(ViewDOMBangHandler.APP_MNEM_PARAM, pSerialisationContext.getThreadInfoProvider().getThreadAppMnem());
      ...
      pSerialiser.append("<div id=\"contextLabelData\" style=\"display:none;\"><span>Context Labels</span><ul>");
      ...
      pSerialiser.append("</ul><span>Variables</span><ul>");

      //Add variable info - TODO - this should probably be done on demand to improve performance
      lDevToolbarContext.getXPathVariableNames().stream().sorted().forEach(v -> {
        pSerialiser.append("<li><strong>$");
        pSerialiser.append(v);
        pSerialiser.append("</strong> - <pre>");
        StringBuffer lResultSB = new StringBuffer();
        try {
          pSerialisationContext.getContextUElem().extendedXPathResult(pSerialisationContext.getContextUElem().attachDOM(), "$"+v).printResultAsXML(lResultSB);
        }
        catch (ExActionFailed e) {
          pSerialiser.append("(evaluation failed: " + e.getMessage() +")");
        }
        pSerialiser.append(StringEscapeUtils.escapeHtml4(lResultSB.toString()));
        pSerialiser.append("</pre></li>");
      });
